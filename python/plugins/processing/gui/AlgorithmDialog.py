...
class AlgorithmDialog(AlgorithmDialogBase):
        ...
        try:
            ...
            if self.iterateParam:
                ...
                while(self.notFinished):
                    self.setInfo(
                        self.tr('<b>Main thread waiting: %d...</b>')%i)
                    time.sleep(2)
                    i+=1
                    
                self.notFinished = True
                # ----------------------------------
                if self.algExeResult:
                    self.finish()
                else:
                    QApplication.restoreOverrideCursor()
                    self.resetGUI()
    ...
    def newGeoAlgInstance(self, alg):
        newInstance = GeoAlgorithm()   
        newInstance.__dict__ = alg.__dict__
        return newInstance
    
    def setAlgExeResult(self):
        self.algExeResult = self.algExecutor.result
